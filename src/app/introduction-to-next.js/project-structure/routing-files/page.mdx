...
| File/Folder       | Purpose                                                                                                                                  |
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `layout.js`       | Defines the layout for a specific route or group of routes, allowing for consistent structure across pages.                              |
| `page.js`         | Represents the main content of a route, rendering the page's content when accessed.                                                      |
| `not-found.js`    | Handles cases where a requested route does not exist, providing a custom 404 page for better user experience.                            |
| `error.js`        | Handles errors that occur within a specific route, providing a custom error page for better user experience.                             |
| `global-error.js` | Handles unhandled errors in the application, providing a consistent error page for the entire application.                               |
| `loading.js`      | Displays a loading indicator while data is being fetched for a specific route, improving user experience during data loading operations. |
| `template.js`     | Defines a reusable template for pages that share similar layouts or functionalities, promoting code reusability.                         |
| `default.js`      | Defines a fallback route for directories, ensuring that users always have a valid page to access.                                        |
| `route.js`        | Defines the API endpoint                                                                                                                 |
